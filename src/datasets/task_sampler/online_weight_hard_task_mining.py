...
class OWHTMSampler(RandomSampler):
    def __init__(self, data_source, tasks, batch_size):
        ...
        with warnings.catch_warnings():
            ...
            super(OWHTMSampler, self).__init__(data_source,
                                              replacement=True)
...
class MetaDatasetRandomSampler(OWHTMSampler):
    def __iter__(self):
        num_classes_per_task = self.data_source.num_classes_per_task
        if len(self.tasks):
            x = self.tasks
...
class MetaDataLoader(DataLoader):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None,
        ...
        if isinstance(dataset, CombinationMetaDataset) and (sampler is None):
            sampler = OWHTMSampler(dataset, task, batch_size)
        shuffle = False
...
class BatchMetaDataLoaderOWHTM(MetaDataLoader):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None, num_workers=0,
        ...
        super(BatchMetaDataLoaderOWHTM, self).__init__(dataset,
...
class OWHTM(object):
    ...
    def __next__(self):
        ...
        for batch in BatchMetaDataLoaderOWHTM(self.dataset,
