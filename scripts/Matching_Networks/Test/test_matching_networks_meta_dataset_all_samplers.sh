...
cd .. && python -m src.main --exp_name test_matching_networks_meta_dataset --log-test-tasks --model matching_networks --runs 1 --folder $SLURM_TMPDIR/records --task_sampler $SLURM_ARRAY_TASK_ID --dataset meta_dataset --num-ways 5 --num-shots 1 --use-cuda --step-size 0.4 --batch-size 1 --num-workers 0 --num-epochs 100 --output-folder ./config/matching_networks_meta_dataset/$SLURM_ARRAY_TASK_ID/
